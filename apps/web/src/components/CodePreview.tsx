...
export interface CodePreviewProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  language?: "typescript" | "javascript" | "bash" | "json" | "env" | "tsx" | "jsx" | "css";
...
function tokenizeBash(line: string): Token[] {
  ...
  parts.forEach((part, i) => {
    ...
    } else if (i === 0 || ["npm", "npx", "yarn", "pnpm", "git", "cd", "mkdir", "touch", "echo", "curl"].includes(part)) {
...
export interface TerminalBlockProps extends Omit<CodePreviewProps, "language" | "variant"> {
  label?: string;
}
...
export const HeroCodeBlock = React.forwardRef<HTMLDivElement, HeroCodeBlockProps>(
  ({ children, ...props }, ref) => (
